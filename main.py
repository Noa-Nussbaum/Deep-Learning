...
if __name__ == '__main__':
    ...
    with tf.Session() as sess:
        ...
        for i in range(epochs):
            ...
            if i%100==0:
                print("i =",i,sess.run(loss, feed_dict={X: xtrain, Y: yListTrain}))
...
#         Let's add a neural network
    ...
    # Layer 2
    layer_2 = 34
    W2 = tf.Variable(tf.truncated_normal([layer_1, layer_2], stddev=0.01))
    ...
    # Layer 3
    layer_3 = 300
    W3 = tf.Variable(tf.truncated_normal([layer_2, layer_3], stddev=0.01))
    ...
    # # Layer 4
    layer_4 = 100
    W4 = tf.Variable(tf.truncated_normal([layer_3, layer_4], stddev=0.01))
    ...
    # Layer 5
    layer_5 = 70
    W5 = tf.Variable(tf.truncated_normal([layer_4, layer_5], stddev=0.01))
    b5 = tf.Variable(tf.constant(0.1, shape=[layer_5]))
    z5 = tf.add(tf.matmul(a4, W5), b5)
    a5 = tf.nn.elu(z5)

    # Output layer 5
    W6 = tf.Variable(tf.truncated_normal([layer_5, classes], stddev=0.01))
    ...
    with tf.Session() as sess:
        ...
        for i in range(epochs):
            ...
            if i % 100 == 0:
                print("i =", i, "Loss", sess.run(lossLayers, feed_dict={xholder: xtrain, yholder: yListTrain}))
        ...
        print("Loss test", sess.run(lossLayers, feed_dict={xholder: xtrain, yholder: yListTrain}))
